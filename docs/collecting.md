...
**처음이라면 §1부터.** 설정할 게 거의 없다 — 로컬 Claude Code를 그대로 쓴다.

| 궁금한 것 | 보는 곳 |
|---|---|
| 수집 범위를 바꾸고 싶다 | 앱 → 소스 → **범위** |
| 밀린 기록을 다 가져오고 싶다 | 앱 → 소스 → **끝까지 수집** |
...
## 1. 처음 한 번 — 내 Claude 세션 연결하기

동료가 이 리포를 처음 열었을 때 여기부터 하면 된다. **설정할 게 사실상 없다**는
게 요점이다 — 로컬에 이미 있는 Claude Code를 그대로 쓰기 때문에 API 키도,
OAuth도, 계정 연결도 없다.

### 1-1. Claude Code가 로그인돼 있는지

```bash
claude --version          # 설치 확인
echo "ping" | claude -p   # 응답이 오면 로그인된 것
```

두 번째 명령이 조용히 끝나거나 에러가 나면, `claude`를 한 번 대화형으로 실행해
로그인만 마치면 된다. knows-me는 **이 CLI의 인증을 그대로 빌려 쓴다** — 별도
키를 두지 않는 이유이자, 팀원마다 각자의 Claude 구독으로 돌아가는 이유다.

### 1-2. 세션은 이미 디스크에 있다

Claude Code는 대화를 알아서 기록해 둔다. 연결이라고 할 것도 없이, 이 경로들이
있으면 그대로 읽는다:
...
없으면 조용히 건너뛴다. 다른 곳에 있다면 §4의 `KNOWSME_SESSION_ROOTS`로 지정한다.
...
기본값 그대로도 돈다. 바꿀 만한 건 모델 하나뿐이다:

| 값 | 언제 |
|---|---|
| `claude-opus-5` | 추출 품질이 중요할 때 (권장) |
| `claude-sonnet-5` | 균형 |
| `claude-haiku-4-5` | 속도가 급할 때 |

> 같은 15개 세션 A/B 측정: **opus-5가 확정 사실 23건, sonnet-5가 7건**
> (걸러냄 5 vs 11). 소요 시간 차이는 8%(353초 vs 325초)에 그쳤다.
> 세션 하나에 에이전트 세션이 뜨는 구조라, 지연은 토큰 수보다 기동 비용이
> 지배한다 — 큰 모델의 대가가 생각보다 싸다.
...
앱에서 볼트 비밀번호를 정하면(첫 실행) 그때부터 저장되는 모든 것이 암호화된다.

소스 화면에 버튼이 세 개 있다:

| 버튼 | 하는 일 |
|---|---|
| **범위** | 어떤 프로젝트의 세션을 모을지 고른다. 아무것도 안 고르면 전부 |
| **새로 온 것만** | 각 소스에서 한 묶음(세션 30개)만. 금방 끝난다 |
| **끝까지 수집** | 남은 기록을 끝까지. 처음이라면 오래 걸린다 |

**처음이라면 「범위」로 관심 있는 프로젝트만 고른 뒤 「끝까지 수집」**을 권한다.
세션 하나에 모델 호출이 5~6번 붙으므로(요약 1회 + 뽑힌 사실마다 분류 1회),
세션 100개면 **40분에서 한 시간**쯤 걸린다. 눌러 두고 다른 일을 하면 된다.

그다음부터는 「새로 온 것만」으로 충분하다.

### 1-5. 내 데이터가 어디로 가나

정직하게 말하면, **텍스트는 Anthropic에 도달한다.** 로컬 CLI를 거칠 뿐이다.
대신 이 세 가지가 보장된다:

- 나가기 전에 이메일·전화번호 등 식별자는 **마스킹**된다. 답변은 기기 안에서
  원문으로 복원된다.
- 나간 기록은 전부 **전송 투명성 로그**에 남는다 (앱에서 볼 수 있다).
- 볼트는 **로컬 파일**이고 AES-256-GCM으로 암호화된다. 스냅샷이 어디로도
  올라가지 않는다.

## 2. 평소 수집 — 앱의 「수집」 버튼

「새로 온 것만」은 세션 루트(`~/.claude/projects`, `~/.codex/sessions`)를 훑어
**한 번에 최대 30개** 파일을 가져온다. 한 번에 다 안 가져오는 건 의도된 것이다 —
가공 한 건마다 LLM 왕복이 여러 번 붙기 때문에, 수백 개짜리 디렉터리를 한 번에
돌리면 한 시간이 넘는다. 그걸 원할 때 쓰는 게 「끝까지 수집」이고, 이쪽은 배치가
비워질 때까지 알아서 반복한다.

> 한때 이 상한이 **볼트가 가질 수 있는 양의 천장**이기도 했다. 디스크에
> 431개가 있는데 한 번에 30개면, 나머지는 버튼을 열다섯 번 누르는 것 말고는
> 도달할 방법이 없었다. 「끝까지 수집」이 그 천장을 없앤다.
...
"처리 완료"로 표시되어 영영 들어오지 않는다.** 특정 세션만 넣고 싶으면 §5를 쓴다.
...
### 5-1. 먼저 스크래치 볼트에서 확인 (`ingest_roots`)
...
`ingest_roots`는 **한 배치(30개)만** 본다. 세션 하나를 확인하는 용도라 그걸로
충분하다. 끝까지 넣으려면 §5-2를 쓴다.

### 5-1½. 진행 상황 보기 — 터미널은 끝날 때까지 조용하다
...
### 5-2. 앱 볼트에 주입 (`ingest_into_vault`)
...
지정한 루트를 **끝까지** 돈다 — 배치가 비워질 때까지 커서를 이어 넘긴다. 끝나면
앱을 다시 띄우고 인터뷰 큐에서 확인하면 확정 사실이 된다.

> 대부분은 이 예제가 필요 없다 — 앱의 **범위 + 끝까지 수집**이 같은 일을 한다.
> 여기 남겨 두는 건 앱을 열지 않고 진단하거나 스크립트로 돌릴 때를 위한 것이다.
