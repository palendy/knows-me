...
**모델이 thinking·reasoning 계열이면** 답보다 추론을 먼저 뱉으므로 토큰 예산이 중요하다. 한 호출당 16384 토큰을 요청한다. 게이트웨이 모델의 최대 출력이 그보다 작아 `400`이 나오면 `LLM_MAX_TOKENS=8192`처럼 낮춰서 실행한다 ([`usage.md` §2-8](usage.md#2-8-한-번에-요청하는-토큰-예산-llm_max_tokens)).
