...
### 2-8. 한 번에 요청하는 토큰 예산 (`LLM_MAX_TOKENS`)

요약·분류 호출은 답변 예산으로 **16384 토큰**을 요청한다. 답 자체는 라벨 몇 개나 몇 문장이라 작지만, thinking 모델은 답보다 **추론 블록을 먼저** 뱉고 그만큼 예산을 먼저 쓴다. 예산이 추론 중에 끝나면 답이 아예 오지 않는다. 예산은 상한이지 소비량이 아니라서, 남는 만큼 요금이 들지는 않는다.

**모델의 최대 출력이 이보다 작으면 요청이 400으로 거부될 수 있다.** 그럴 때만 환경변수로 낮춘다.

```bash
LLM_MAX_TOKENS=8192 npx tauri dev
```

숫자가 아니거나 0이면 무시하고 기본값을 쓴다.
...
**"LLM response hit the N-token limit".** thinking 모델이 추론에 예산을 다 썼다. 답이 나오기 전에 예산이 끝나 잘린 것이다. 한 번에 요청하는 예산은 **16384 토큰**이고, 모자라면 LM Studio에서 thinking을 끄거나 더 작은 모델을 쓴다. 해당 항목은 사라지지 않고 대기 큐에 적재돼 다음에 다시 시도된다.

**"LLM API 400" — max_tokens가 모델 한도를 넘는다고 한다.** 모델의 최대 출력이 16384보다 작으면 요청 자체가 거부된다. 그 모델의 한도에 맞춰 `LLM_MAX_TOKENS`를 낮춰서 앱을 실행한다 (§2-8).
