...
### MCP 벌티컬 — 오너(나) solo
> **상태: 구현 완료** (단계 1~5 머지, PR #7/#9/#11/#13). 코드 `src-tauri/src/sharing/`, 요약 [`aidlc-docs/construction/sharing-mcp-vertical/code/summary.md`](../aidlc-docs/construction/sharing-mcp-vertical/code/summary.md). 터널은 cloudflared 퀵터널로 확정. 계약 §10 열린 항목 해소는 `mcp-contract.md` §11.
...
- **완료(달성)**: ⓐ 오너 localhost 셀프 질의 → ⓑ 토큰으로 남도 scoped 질의. 둘 다 동작(실 터널 스모크 통과).
...
2. **오너: MCP 벌티컬 solo 착수** — 전송·토큰·스코프 → 툴 로직 → 터널. (분류는 수집+활용 담당이 상류에서.) ✅ **완료**(PR #7/#9/#11/#13).
