...
const systemPrompt = (ownerName) =>
  ...
  `search_knowledge 결과가 비면 포기하지 말고, list_categories로 질문과 관련 있어 보이는 범주를 찾아 ` +
  `get_guide로 그 범주의 페이지들을 가져와 다시 확인하라(검색이 substring 기반이라 표현이 다르면 안 걸릴 수 있다). ` +
  `그렇게 해도 근거가 없을 때만 "공개 범위에 없다"고 답하라. ` +
