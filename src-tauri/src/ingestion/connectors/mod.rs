...
//!
//! Full implementations: [`session`], [`file`] (text/markdown), and — behind
//! their HTTP features — [`notion`] (`notion-http`), [`confluence`] and
//! [`jira`] (`atlassian-http`, shared plumbing in [`atlassian`]).
//! Skeletons (INTEGRATION-TODO): [`gmail`] and the PDF/DOCX/vision branches
//! inside [`file`] — they satisfy the contract and build offline, but the real
//! external calls / heavy parsers are wired up during integration.
