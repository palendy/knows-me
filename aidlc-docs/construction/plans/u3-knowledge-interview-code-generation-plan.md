...
- [x] **Step 1 — Module scaffolding**: create `knowledge/mod.rs`, `interview/mod.rs`; add `pub mod knowledge; pub mod interview;` to `src-tauri/src/lib.rs` (additive; U1 gate-keeps shared file). *(US-3.x, US-4.x)*

- [x] **Step 2 — Repository layer** (`fact_store.rs`, `history.rs`, `queue_manager.rs`):
