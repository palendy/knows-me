...
터널은 배관일 뿐, 인증을 대신해주지 않는다. **토큰-스코프 인증은 서버가 강제**한다. 토큰은 고엔트로피 랜덤, per-consumer 발급/폐기.
...
보안은 로컬 전용으로 시작한다. 비밀번호에서 Argon2id로 키를 유도하고, 평문 키는 디스크에 남기지 않으며(메모리에서 zeroize), 맥락 데이터와 외부 자격증명은 모두 암호화 저장한다. LLM 클라우드 호출은 기본 비활성(`llm-http` feature)이고, 켜더라도 전송 전 식별자를 마스킹하고 전송 내역을 로그로 남긴다. **팀 공유(MCP 스코프 제공)는 강화 컨셉의 다음 단계다 — 아래 우선순위 참고.**
...
1. **ⓐ 오너 셀프 참조부터** — MCP 서버(Streamable HTTP) + 토큰-스코프 인증 + localhost 질의. 가장 확실한 가치.
2. **ⓑ 터널+토큰 공유로 확장** — 같은 서버에 cloudflared/Tailscale를 얹는다. 뼈대는 그대로.
3. 상시 가용(배포)은 데이터-off-device 문제를 다시 여니 나중 단계.
...
같은 팀의 선행 프로젝트 [`dev-team-404/space-a`](https://github.com/dev-team-404/space-a)가 이 컨셉의 상당 부분을 이미 구현했다. 특히 **토큰 파생·서버 강제 스코프**(`a-hub/work/ahub/core/services.py`), **한 프로세스 MCP+REST 제공**(ADR 0002), **전송 경계 논증**(ADR 0019/0025), **`get_guide`로 온보딩 대신** 패턴을 참고한다. 단, space-a가 스스로 밝힌 보안 미비(추측 가능 토큰 `tok_N`, 무인증 경로, 클라이언트가 tier 자가선언, opt-in 없는 자동 발행, 취소 불가 잔존)는 knows-me의 존재 이유와 정면 충돌하므로 상속하지 않는다.

## 상태

**컨셉 강화 (draft)** — 2026-09-09. 최초 초안(세션 수집 → CLAUDE.md)에서 **개인 지식 위키 + 공용/개인 분류 + MCP 스코프 제공**으로 컨셉을 조였다. 4개 유닛(U1~U4) 라이브러리는 초안 기준으로 구현·통합 완료. 강화된 컨셉에 맞춘 기능 강화(MCP 서버·분류·공유)는 설계 착수 전.
